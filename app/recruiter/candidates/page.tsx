...
import {
  Search, X, Loader2, AlertCircle, Calendar,
  MessageSquare, CheckCircle, XCircle, User,
...
const STATUS_CONFIG: Record<ApplicationStatus, { label: string; color: string; bg: string }> = {
  APPLIED:             { label: "Applied",              color: "text-blue-700 dark:text-blue-300",    bg: "bg-blue-50 dark:bg-blue-900/20" },
  SCREENING:           { label: "Screening",            color: "text-sky-700 dark:text-sky-300",      bg: "bg-sky-50 dark:bg-sky-900/20" },
  REVIEWING:           { label: "Reviewing",            color: "text-amber-700 dark:text-amber-300",  bg: "bg-amber-50 dark:bg-amber-900/20" },
  INTERVIEW_SCHEDULED: { label: "Interview Scheduled",  color: "text-violet-700 dark:text-violet-300",bg: "bg-violet-50 dark:bg-violet-900/20" },
  OFFER:               { label: "Offer",                color: "text-indigo-700 dark:text-indigo-300",bg: "bg-indigo-50 dark:bg-indigo-900/20" },
  HIRED:               { label: "Hired",                color: "text-emerald-700 dark:text-emerald-300",bg:"bg-emerald-50 dark:bg-emerald-900/20" },
  ACCEPTED:            { label: "Accepted",             color: "text-emerald-700 dark:text-emerald-300",bg:"bg-emerald-50 dark:bg-emerald-900/20" },
  REJECTED:            { label: "Rejected",             color: "text-red-600 dark:text-red-400",      bg: "bg-red-50 dark:bg-red-900/20" },
  WITHDRAWN:           { label: "Withdrawn",            color: "text-gray-500",                        bg: "bg-gray-100 dark:bg-gray-800" },
};

const FILTER_STATUSES: ApplicationStatus[] = [
  "APPLIED","REVIEWING","INTERVIEW_SCHEDULED","ACCEPTED","REJECTED",
];
...
export default function RecruiterCandidatesPage() {
  ...
  const [interviewForm, setInterviewForm] = useState({
    scheduledAt: "", type: "VIDEO" as "VIDEO"|"PHONE"|"IN_PERSON",
    notes: "", meetingLink: "",
  });
  ...
  // ── Status update ────────────────────────────────────────────────────────
  const updateStatus = async (candidateId: string, status: ApplicationStatus) => {
    ...
    try {
      ...
      setSelected((prev)   => prev?._id === candidateId ? { ...prev, status } : prev);
    } catch {
  ...
  const handleSchedule = async () => {
    ...
    try {
      ...
      setInterviewForm({ scheduledAt: "", type: "VIDEO", notes: "", meetingLink: "" });
    } catch (err: any) {
  ...
  return (
    <div className="flex h-[calc(100vh-10rem)] gap-6">
      ...
      {/* Left: list */}
      <div className={`flex flex-col ${selected ? "w-full lg:w-2/5" : "w-full"} transition-all`}>
        <div className="mb-4">
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pr-1">
          {isLoading && <div className="flex items-center justify-center py-16"><Loader2 size={28} className="animate-spin text-gray-400" /></div>}

          {!isLoading && error && (
            <div className="flex flex-col items-center justify-center py-16 text-red-400 gap-2">
              <AlertCircle size={28} />
              <p className="text-sm">{error}</p>
              <button onClick={fetchCandidates} className="text-xs underline mt-1">Retry</button>
            </div>
          )}

          {!isLoading && !error && candidates.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-gray-400">
              <User size={36} className="mb-3 text-gray-300 dark:text-gray-700" />
            ...
            return (
              <div key={c._id}
                ...
                className={`bg-white dark:bg-gray-900 rounded-2xl border p-4 cursor-pointer transition-all shadow-sm hover:shadow-md ${
                  isSelected ? "border-indigo-400 dark:border-indigo-600 ring-2 ring-indigo-200 dark:ring-indigo-800" : "border-gray-100 dark:border-gray-800"
                }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <span className="text-xs text-gray-400">
                        {c.appliedAt ? new Date(c.appliedAt).toLocaleDateString("en-US", { month: "short", day: "numeric" }) : "—"}
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="p-6 border-b border-gray-100 dark:border-gray-800">
              <div className="flex items-start justify-between gap-4">
                ...
                <button onClick={() => setSelected(null)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl cursor-pointer">
                  <X size={18} />
                </button>
              </div>

              <div className="mt-4 flex items-center gap-2">
                <span className={`px-3 py-1 rounded-full text-sm font-medium ${STATUS_CONFIG[selected.status].color} ${STATUS_CONFIG[selected.status].bg}`}>
                  {STATUS_CONFIG[selected.status].label}
                </span>
                <span className="text-sm text-gray-400">
                  Applied {selected.appliedAt
                    ? new Date(selected.appliedAt).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })
                    : "—"}
                </span>
              </div>
            </div>
            ...
            <div className="p-6 space-y-5">
              {selected.skills && selected.skills.length > 0 && (
                <div>
                  <h3 className="font-bold text-gray-900 dark:text-white mb-2 text-sm">Skills</h3>
                  <div className="flex flex-wrap gap-2">
                    {selected.skills.map((s) => (
                      <span key={s} className="px-2.5 py-1 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300 rounded-lg text-xs font-medium">
                        {s}
                      </span>
                    ))}
                  </div>
          ...
          <div className="p-4 border-t border-gray-100 dark:border-gray-800">
            <div className="flex gap-2 flex-wrap mb-3">
              {selected.status === "APPLIED" && (
                <button onClick={() => updateStatus(selected._id, "REVIEWING")} disabled={!!updatingId}
              ...
              {(selected.status === "APPLIED" || selected.status === "REVIEWING") && (
                <button onClick={() => openScheduleModal(selected)}
              ...
              {!["ACCEPTED","HIRED","REJECTED","WITHDRAWN"].includes(selected.status) && (
                <button onClick={() => updateStatus(selected._id, "ACCEPTED")} disabled={!!updatingId}
              ...
              {!["REJECTED","WITHDRAWN"].includes(selected.status) && (
                <button onClick={() => updateStatus(selected._id, "REJECTED")} disabled={!!updatingId}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md border border-gray-200 dark:border-gray-800">
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 dark:border-gray-800">
              ...
              <button onClick={() => { setShowScheduleModal(false); setScheduleError(null); }}
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <select value={interviewForm.type}
                  onChange={(e) => setInterviewForm((f) => ({ ...f, type: e.target.value as any }))}
                  className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 text-gray-900 dark:text-white">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Notes for Candidate</label>
                <textarea rows={3} placeholder="Preparation instructions, address, etc."
