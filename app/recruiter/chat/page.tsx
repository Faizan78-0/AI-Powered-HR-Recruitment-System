...
import {
  ...
  Paperclip, Smile, CheckCheck, ArrowLeft, Loader2,
} from "lucide-react";
...
export default function RecruiterChatPage() {
  ...
  // ── Select conversation — lazy-load messages ──────────────────────────
  const selectConvo = useCallback(
    async (id: string) => {
      ...
      setLoadingMsgs(true);
      try {
        const res  = await fetch(`/api/recruiter/chat?conversationId=${id}`);
        const data = await res.json();
        setConvos((prev) =>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {!loadingList && filtered.length === 0 && (
            <p className="text-center text-sm text-gray-400 mt-10 px-6">
              {convos.length === 0
                ? "No conversations yet. Accept a candidate to start chatting."
                : "No matches found."}
            </p>
          )}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              {!loadingMsgs && selected.messages.length === 0 && (
                <p className="text-center text-sm text-gray-400 mt-8">
                  Say hello to {selected.name.split(" ")[0]}!
                </p>
              )}
