...
import {
  ...
  Send, X, Loader2, AlertCircle, ChevronDown,
} from "lucide-react";
...
interface Job {
  ...
  postedDate:        string | null;  // mapped from createdAt by API
  remote:            boolean;
...
const STATUS_BADGE: Record<ApplicationStatus, { label: string; className: string }> = {
  APPLIED:             { label: "✓ Applied",             className: "text-indigo-600 dark:text-indigo-400" },
  SCREENING:           { label: "🔍 Screening",           className: "text-blue-600 dark:text-blue-400" },
  REVIEWING:           { label: "👁 In Review",            className: "text-amber-600 dark:text-amber-400" },
  INTERVIEW_SCHEDULED: { label: "📅 Interview Scheduled", className: "text-violet-600 dark:text-violet-400" },
  OFFER:               { label: "🎁 Offer Received",      className: "text-sky-600 dark:text-sky-400" },
  HIRED:               { label: "🎉 Hired",               className: "text-emerald-600 dark:text-emerald-400" },
  ACCEPTED:            { label: "🎉 Accepted",            className: "text-emerald-600 dark:text-emerald-400" },
  REJECTED:            { label: "✗ Not Selected",         className: "text-red-500 dark:text-red-400" },
  WITHDRAWN:           { label: "Withdrawn",              className: "text-gray-400" },
};

// Active statuses where the user has a pending/ongoing application
const ACTIVE_STATUSES: ApplicationStatus[] = ["APPLIED", "SCREENING", "REVIEWING", "INTERVIEW_SCHEDULED", "OFFER"];
...
// FIX: No seekerId prop — auth is resolved server-side from the JWT cookie.
//      The old version required a seekerId prop that was never reliably passed.
export default function JobSeekerJobsPage() {
  ...
  const fetchJobs = useCallback(async () => {
    try {
      ...
      // Auto-select first job on first load (desktop convenience)
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      // Optimistic update — mark job as APPLIED in the list and detail panel
      const patch = (j: Job): Job =>
        j.id === applyingJob.id
          ? { ...j, applicationStatus: "APPLIED", applicationId: data.applicationId ?? data.id }
          : j;
  ...
  const handleWithdraw = async (job: Job) => {
    ...
    try {
      const res = await fetch(`/api/jobseeker/jobs?id=${job.applicationId}`, { method: "DELETE" });
      if (!res.ok && res.status !== 204) throw new Error();
      const patch = (j: Job): Job =>
        j.id === job.id ? { ...j, applicationStatus: null, applicationId: null } : j;
      setJobs((prev) => prev.map(patch));
      setSelectedJob((prev) => prev ? patch(prev) : prev);
    } catch {
  ...
  return (
    <div className="flex h-[calc(100vh-10rem)] gap-6">
      ...
      {/* ── Left: Job List ── */}
      <div className={`flex flex-col ${selectedJob ? "w-full lg:w-2/5" : "w-full"} transition-all`}>
        <div className="mb-4">
        ...
        <div className="space-y-3 mb-4">
          ...
          <div className="flex gap-2 flex-wrap">
            {(["ALL", "FULL_TIME", "PART_TIME", "CONTRACT", "FREELANCE"] as const).map((t) => (
              <button key={t}
            ...
            <button onClick={() => { setRemoteFilter(!remoteFilter); setPage(1); }}
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pr-1">
          ...
          {!isLoading && error && (
            <div className="flex flex-col items-center justify-center py-16 text-red-400 gap-2">
              ...
              <button onClick={fetchJobs} className="text-xs underline mt-1">Retry</button>
            </div>
          )}
            ...
            return (
              <div key={job.id}
                ...
                <div className="flex items-start gap-3">
                  <div className={`w-10 h-10 rounded-xl bg-linear-to-br ${logoGradient(job.company)} flex items-center justify-center text-white font-bold text-sm shrink-0`}>
                    {job.company.slice(0, 2).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-sm text-gray-900 dark:text-white truncate">{job.title}</p>
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      {job.applicationStatus && (
                        <span className={`text-xs font-semibold ${STATUS_BADGE[job.applicationStatus].className}`}>
      ...
      {selectedJob && (
        <div className="hidden lg:flex lg:flex-1 flex-col bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
            ...
            <div className="p-6 border-b border-gray-100 dark:border-gray-800">
              <div className="flex items-start justify-between gap-4 mb-4">
                <div className="flex items-start gap-4">
                  <div className={`w-14 h-14 rounded-2xl bg-linear-to-br ${logoGradient(selectedJob.company)} flex items-center justify-center text-white font-bold text-lg shrink-0`}>
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 dark:text-gray-400">{selectedJob.company}</p>
                    {selectedJob.applicationStatus && (
                      <span className={`text-sm font-semibold ${STATUS_BADGE[selectedJob.applicationStatus].className}`}>
              ...
              <div className="flex flex-wrap gap-3 text-sm text-gray-600 dark:text-gray-400">
                {selectedJob.location && <span className="flex items-center gap-1.5"><MapPin size={14} /> {selectedJob.location}</span>}
                <span className="flex items-center gap-1.5"><Briefcase size={14} /> {TYPE_LABEL[selectedJob.type] ?? selectedJob.type}</span>
                {selectedJob.salary && <span className="flex items-center gap-1.5"><DollarSign size={14} /> {selectedJob.salary}</span>}
                {selectedJob.postedDate && (
                  <span className="flex items-center gap-1.5">
                    <Clock size={14} /> Posted {new Date(selectedJob.postedDate).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                  </span>
                )}
                {selectedJob.remote && (
                  <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedJob.requiredSkills.map((skill) => (
                      <span key={skill} className="px-3 py-1.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 text-sm font-medium rounded-lg">
                        {skill}
                      </span>
                    ))}
                  </div>
          ...
          <div className="p-4 border-t border-gray-100 dark:border-gray-800 flex gap-3">
            {selectedJob.applicationStatus === "ACCEPTED" || selectedJob.applicationStatus === "HIRED" ? (
            ...
            ) : ACTIVE_STATUSES.includes(selectedJob.applicationStatus as ApplicationStatus) ? (
              <>
                ...
                <button onClick={() => handleWithdraw(selectedJob)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg border border-gray-200 dark:border-gray-800 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 dark:border-gray-800 sticky top-0 bg-white dark:bg-gray-900 z-10">
              <div>
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">Apply for Position</h3>
                <p className="text-sm text-gray-500 mt-0.5">{applyingJob.title} · {applyingJob.company}</p>
              </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Resume / CV Link <span className="text-gray-400 font-normal">(optional)</span>
                </label>
                <input type="url" value={resumeUrl}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Key Skills <span className="text-gray-400 font-normal">(optional)</span>
                </label>
                ...
                {skills.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-2">
                    {skills.map((s) => (
                      <span key={s} className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300 rounded-lg text-xs font-medium">
                        {s}
                        <button onClick={() => removeSkill(s)} className="hover:text-red-500 transition-colors cursor-pointer">
                          <X size={12} />
                        </button>
                      </span>
                    ))}
              ...
              {/* Pre-fill hint if job has required skills */}
              {applyingJob.requiredSkills?.length > 0 && skills.length === 0 && (
                <div className="px-4 py-3 bg-amber-50 dark:bg-amber-900/20 rounded-xl text-xs text-amber-700 dark:text-amber-300">
                  <span className="font-semibold">Tip:</span> This role requires: {applyingJob.requiredSkills.slice(0, 5).join(", ")}
                  {applyingJob.requiredSkills.length > 5 ? ` +${applyingJob.requiredSkills.length - 5} more` : ""}
                </div>
              )}
            </div>
