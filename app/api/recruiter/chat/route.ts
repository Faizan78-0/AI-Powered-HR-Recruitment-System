...
function shapeMessage(m: any, recruiterId: string) {
  const isMe = m.senderRole === "Recruiter";
  return {
    id:             m._id.toString(),
    text:           m.text,
...
// ?conversationId=  → single convo with messages (marks seeker msgs as read)
// (no param)        → conversation list, no messages
  ...
  try {
    ...
    if (convId) {
      ...
      const conv = await Conversation.findOne({
        ...
        .populate({ path: "seekerId",      model: User,        select: "Name email imageUrl" })
        .populate({
          path:  "applicationId",
          model: Application,
      ...
      conv.messages.forEach((m: any) => {
        if (m.senderRole === "Job seeker" && !m.read) {
          m.read  = true;
          changed = true;
        }
      });
      ...
      const seeker = conv.seekerId as any;
      const job    = (conv.applicationId as any)?.jobId;

      return NextResponse.json({
        id:          conv._id.toString(),
        name:        seeker?.Name  ?? "Candidate",
    ...
    const shaped = (convs as any[]).map((c) => ({
      id:          c._id.toString(),
      name:        c.seekerId?.Name  ?? "Candidate",
      role:        (c.applicationId as any)?.jobId?.title ?? "Applicant",
      online:      false,
      unread:      c.unreadRecruiter ?? 0,
...
// Marks all seeker messages as read (for manual read-receipt calls)
export async function PATCH(req: NextRequest) {
  try {
    ...
    conv.messages.forEach((m: any) => {
      if (m.senderRole === "Job seeker") m.read = true;
    });
    conv.unreadRecruiter = 0;
    await conv.save();
